...
export default function AlimentationPage() {
  ...
  const todaysMeals = meals.filter(meal => {
    ...
    return mealDate === today && meal.petId === currentPet?.id;
  });
  ...
  return (
    <div className="pb-28 px-4 py-8">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold text-emerald-900 mb-6">Suivi Alimentation</h1>

        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Type de Repas</label>
              <select
                value={mealType}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Quantité (g)</label>
              <input
                type="number"
                value={quantity}
            ...
            {todaysMeals.length === 0 ? (
              ...
              <div className="space-y-2">
                {todaysMeals.map(meal => (
                  <div key={meal.id} className="flex items-center justify-between bg-emerald-50 px-4 py-3 rounded-lg">
